  ...
  handleSimulationEvent: (event) => {
    ...
    if (event.event.NodeUpdate) {
      ...
      set((state) => {
        ...
        if (nodeExists) {
          const isDetached = update.role === "Detached" || update.role === "detached";
          return {
            nodes: state.nodes.map((n) =>
              n.id === update.node_id.toString()
                ? {
                    ...
                    data: {
                      ...n.data,
                      role: isDetached ? "detached" : update.role,
                      label: `${isDetached ? "detached" : update.role} (Node ${n.id})`,
          ...
          const newNode: Node = {
            ...
            type: "device",
            data: {
              label: `${update.role} (Node ${update.node_id})`,
              role: update.role,
              queueDepth: update.queue_depth,
              queueDelay: update.queue_delay_us,
              rloc16: update.rloc16,
